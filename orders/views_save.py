...
def checkout(request):
	...
	orm = CheckoutContactForm(request.POST or None)
	user = request.user
	...
	if request.POST:
		...
		if form.is_valid():
			...
			#regUsr(request, email, password, form)
...
def regUsr(request, usr, pswd, frm):
	username_qs = User.objects.filter(username=usr)
